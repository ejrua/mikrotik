...
device.connect().then(([login])=>login('api','Ejrua2517')).then(function(conn) {
    ...
    actionChannel.write('/interface/set',{'disabled':'yes','.id':'ether3'}).then(results=>{
        ...
        listenChannel.data
            ...
            .subscribe(d=>actionChannel.write('/interface/set',{'disabled':'no','.id':'ether3'}).then(resolve,reject));
        return promise;
    })
    .then(results=>{
